<script lang="ts">
    ...
    let has_file_loaded = false;
    let setting_destination = DEFAULT_SETTINGS.destination;
    let setting_source_port = DEFAULT_SETTINGS.source_port;
    let setting_ttl = DEFAULT_SETTINGS.ttl;
    let player_state = 'Open a recording';
    let player_position = {
        position: 0,
        max_position: 0,
        time_position: 0,
...
<main class="main-viewport border m-0">
    ...
    <div class="flex flex-row flex-no-wrap justify-evenly w-full py-2">
        <button on:click={play} class="player-control">
        ...
        <button on:click={pause} class="player-control">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="player-control-icon">
                <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zM9 8.25a.75.75 0 00-.75.75v6c0 .414.336.75.75.75h.75a.75.75 0 00.75-.75V9a.75.75 0 00-.75-.75H9zm5.25 0a.75.75 0 00-.75.75v6c0 .414.336.75.75.75H15a.75.75 0 00.75-.75V9a.75.75 0 00-.75-.75h-.75z" clip-rule="evenodd" />
            </svg>
        </button>

        <button on:click={rewind} class="player-control">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="player-control-icon">
        ...
        <button on:click={open_file} class="player-control">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="player-control-icon">
