...
<main class="main-viewport flex flex-col justify-between bg-base-100 border">
    <header class="p-2">
        ...
        <label class="swap swap-rotate absolute top-0 right-0 text-primary-focus p-2">
    ...
    <div class="grow flex justify-between p-0 bg-base-200">
        <div class="flex-auto w-1/2 p-2">
            ...
            {#if has_file_loaded}
                <div class="grid grid-cols-2 gap-4 py-1">
                    ...
                    <label class="label"><span class="label-text">{current_file}</span></label>
        ...
        <SettingsPane on:update={(settings)=>update_settings(settings)} />
    </div>
    ...
    <Controls on:play={play} on:pause={pause} on:rewind={rewind} on:open={open_file} bind:player_state={player_state} />
</main>
